...
def evidence_guard(text: str, candidate_profile: dict[str, Any]) -> dict[str, Any]:
    """Validate complete quantified claims and return readable claim sentences."""
    source = " ".join([
    ...
    generated_sentences = re.split(r"(?<=[.!?])\s+", text or "")
    unsupported: list[str] = []

    metric_pattern = re.compile(
        r"(?<!\w)(?:\d{1,3}(?:[,.]\d{3})*|\d+(?:\.\d+)?)"
        r"(?:\s*(?:%|percent|\+|members?|users?|engineers?|teams?|countries?|transactions?|years?|projects?|applications?))?",
        re.I,
    )
    ...
    for sentence in generated_sentences:
        ...
        # Compare normalized numeric values, not malformed joined fragments.
        normalized_source_numbers = {
            re.sub(r"[^0-9.]", "", value)
            for value in metric_pattern.findall(source)
        ...
        if sentence_numbers and not sentence_numbers.issubset(normalized_source_numbers):
