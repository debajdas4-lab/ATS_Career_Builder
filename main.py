...
def _ensure_v3_artifacts(state: dict, *, company_url: str = "", leadership_url: str = "", market_urls: list[str] | None = None, market_query: str = "") -> dict:
    ...
    requested = bool(company_url or leadership_url or (market_urls or []) or market_query)
    if requested and (not isinstance(research, dict) or not research):
        try:
            state["research"] = build_research_pack(
            ...
            state["research"] = {
                "company_profile": ({
                    ...
                    "overview": "The supplied company URL was received, but detailed public-page research was unavailable during this run.",
                    "url": company_url,
                } if company_url else {}),
                "leadership": [],
                "strategy": [],
                "recent_signals": [],
                "competitors": [],
                "sources": ([{"type": "company", "label": "Company Website", "url": company_url}] if company_url else []),
                "market_signals": [],
                "research_warning": f"Research fallback used: {type(exc).__name__}: {exc}",
            }
