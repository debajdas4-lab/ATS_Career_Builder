"""ATS Career Builder â€” Enterprise Edition (Streamlit UI).

Premium navy/violet theme, hero banner, and fully-working tabs wired to the
FastAPI backend. All content is dynamic â€” nothing about any candidate is
...
st.set_page_config(page_title="ATS Career Guide", page_icon="ðŸŽ¯", layout="wide")
...
# --------------------------------------------------------------------------- #
NAV_ITEMS = [
    ("analyze", "ðŸŽ¯", "Analyze Job"),
    ("resume", "ðŸ“„", "Resume"),
    ("linkedin", "in", "LinkedIn"),
    ("naukri", "âŒ", "Naukri"),
    ("interview", "â—‰", "Interview Kit"),
    ("roadmap", "â†—", "Career Roadmap"),
    ("research", "â—Œ", "Research"),
]
...
def resume_preview(text: str) -> str:
    ...
    for raw in (text or "").splitlines():
        ...
        if line.upper().rstrip(":") in heads:
            ...
            out.append(f"â€¢ {html.escape(line[2:])}<br>")
        else:
...
# --------------------------------------------------------------------------- #
# Sidebar (grouped nav â€” matches original design)                              #
# --------------------------------------------------------------------------- #
with st.sidebar:
    ...
    st.button("ðŸ—‚  My Projects", key="nav_projects", width='stretch')
    st.button("ðŸ“Š  Saved Reports", key="nav_reports", width='stretch')

    st.divider()
    _user = signed_in_user()
    if _user:
        st.caption(f"ðŸ‘¤ Signed in: {_user.get('name')}")
    st.caption("âœ¦ Pro tip: add your LinkedIn & Naukri profile text and a company "
...
jd_url = st.text_input("JD URL", placeholder="Paste a public job URL (LinkedIn, Naukri, careers page)â€¦",
                       label_visibility="collapsed")
st.markdown('<div class="field-label">OR, PASTE JOB DESC. BELOW</div>', unsafe_allow_html=True)
jd_text = st.text_area("JD text", height=150, placeholder="Paste the full job description hereâ€¦",
                       label_visibility="collapsed")

with st.expander("âž• Add company, leadership & market inputs (optional)"):
    c1, c2 = st.columns(2)
    with c1:
...
if st.button("ðŸš€  Analyze job & build career guide", type="primary", width='stretch'):
    if run_disabled:
        st.warning("Please upload your resume and provide a job URL or paste the job description.")
        st.stop()
    with st.status("Building your career guideâ€¦", expanded=True) as status:
        status.write("âœ“ Inputs received")
        status.write("â—Œ Extracting dynamic keywords & candidate profile")
        status.write("â—Œ Scoring resume against the job description")
        status.write("â—Œ Generating premium resume & application artefacts")
        try:
            resp = httpx.post(
...
if result:
    ...
    k2.metric("ATS â€” Current", f"{before} / 100")
    k3.metric("ATS â€” Upgraded", f"{after} / 100", delta=f"{after - before:+d} pts")
    k4.metric("Decision", fit.get("recommendation", "REVIEW"))

    comp = result.get("ats_analysis", {}).get("components", {})
    st.caption(
        f"Score = 65% keyword coverage ({comp.get('keyword_coverage', 0):.0%}) + "
        f"15% semantic match ({comp.get('semantic_similarity', 0):.0%}) + "
        f"20% section completeness ({comp.get('section_completeness', 0):.0%}). "
        f"Resume generation mode: {result.get('generation_mode', 'n/a')}."
    )
    ...
    with tabs[1]:
        ...
        c_txt.download_button("â¬‡ Download TXT", resume_out, "ats-career-guide-resume.txt", "text/plain",
                              width='stretch')
        try:
            docx = httpx.post(f"{API_URL}/v3/export-docx", json={"resume_text": resume_out},
                              headers=auth_headers(), timeout=60)
            docx.raise_for_status()
            c_docx.download_button("â¬‡ Download DOCX", docx.content, "ats-career-guide-resume.docx",
